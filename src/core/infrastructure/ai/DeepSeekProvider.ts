...
export class DeepSeekProvider implements AiProvider {
  ...
  async analyzeTutorial(transcript: string): Promise<AiAnalysisResult> {
    const prompt = `
      ...
      6. Patrones de Rasgueo (strummingPatterns): Identifica los ritmos enseñados. DEBES generar una lista (MÁXIMO 2 patrones). Los símbolos permitidos en el array 'pattern' son SÓLO: "↓" (abajo), "↑" (arriba), "X" (mute/chasquido), "B" (bajo/bordón), "-" (pausa/espacio), "↓X" (abajo con mute/chasquido rápido). ¡NUNCA DEJES ESTE ARREGLO VACÍO! Extrae en 'timeSeconds' el segundo exacto donde se explica o empieza a tocar ese ritmo. Si no sabes qué hacer, pon ["↓", "↓", "↓", "↓"].
      ...
      Devuelve ÚNICAMENTE un objeto JSON con este formato exacto:
      {
        ...
        "strummingPatterns": [
          { "name": "Ritmo Principal", "pattern": ["↓", "↓X", "↑", "↑", "↓X", "↑"], "timeSeconds": 120 },
          { "name": "Ritmo Inferido", "pattern": ["↓", "↓", "↑"], "timeSeconds": 145 }
        ],
        "summary": ["Punto 1..."]
      }
    ...
    console.log("AI RAW JSON RESPONSE:", cleanJson); // Added for debugging
    let result;
    try {
      result = JSON.parse(cleanJson);
    } catch (error) {
      throw new Error("El modelo generó demasiados datos y cortó la respuesta (Límite de tokens). Intenta con un video más corto o cambia a Gemini.");
    }
    ...
    const mappedTimestamps = (result.chordTimestamps || []).map((t: any) => {
      ...
      return t; // fallback if AI still used the old format
    });
    ...
    return {
      ...
      strummingPatterns: result.strummingPatterns || result.StrummingPatterns || [],
